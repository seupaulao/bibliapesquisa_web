...
//  alert(dp[1]);
  ...
  for(var i=0; i<dp.length; i++)
  {
    ...
    w3.removeClass("#s"+dp[i], "w3-orange");
  }
...
{
  var temp = "#s"+getDiaPlanoEstudo();
  alert(temp);
  w3.addClass(temp, "w3-blue");
  w3.removeClass(temp, "w3-orange");
...
calculaPlanoBasico = function (inicial, total, somador, gcap)
{
 ...
 //      dataNac = getNacionalidade()=="pt-BR" ? "Dia " + dia + ": " + getDataPtFormatado(data) : "Day " + dia + ": " +getDataEnFormatado(data);
       dataNac = getNacionalidade()=="pt-BR" ? "Dia " + dia : "Day " + dia ;
       str += "<div id='s"+dia+"'class='w3-col s3 w3-btn w3-padding-16 w3-orange w3-border' onclick='abrirControleLeitura(\""+saidaControle+"\","+dia+")'>" + dataNac + "</div>";
       //addDays(data, 1);
       //var tempstr;
...
obterLista = function(hoje, diaN, diaFS)
{
   var lista = new Array();
   j = hoje;
   for (var i = 0; i < 7; i++)
...
calcularPlanoAlternado = function(inicial, total, somadorDiaNormal, somadorFimSemana, gcap)
{
   ...
   // var data = new Date(t);
    ...
    var lista = obterLista(data.getDay(), somadorDiaNormal, somadorFimSemana);
  
    while(contador <= todos)
    {
       var saidaControle;
       if (data.getDay() == 6 || data.getDay() == 0) saidaControle=acumulaControleLeitura(contador, somadorFimSemana, gcap, todos)
